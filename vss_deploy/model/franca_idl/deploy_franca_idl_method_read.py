...
class DeployFrancaIDLMethodRead:
    __slots__ = (
        'source_method', 'source_attribute',
    ...
    def from_spec(
        cls,
        spec: JSONObject,
    ) -> Optional['DeployFrancaIDLMethodRead']:
        method = spec.get('read')
        if not method:
            return None
        if not isinstance(method, dict):
            raise ValueError(
                'FrancaIDL.methods.read expects a JSON object'
            )
        return cls(method)
